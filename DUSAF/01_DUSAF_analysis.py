...
Analizzare tutte le ATS e non solo le due selezionate
e metodologicamente piu robusto: permette di dimostrare che
ATS Brianza e ATS Montagna rappresentano i due estremi dello
spettro lombardo in termini di presenza industriale tra le
ATS non metropolitane, e che la loro scelta non e arbitraria
ma giustificata da un confronto sistematico su dati reali.
...
Risultati osservati (DUSAF 7, 2021):
    ...
    Rapporto:     ~14x piu industriale Brianza vs Montagna
...
def grafico_barre_tutte_ats(df_all: pd.DataFrame, output_dir: str):
    ...
    decrescente. Evidenzia Bergamo e Montagna con sfondo colorato.
    """
    ...
    # Sfondo colorato per le due ATS selezionate
    for idx, ats in enumerate(ats_labels):
        if ats == "Bergamo":
            ax.axvspan(idx - 0.45, idx + 0.45,
                       color="#ffe0e0", zorder=0, alpha=0.5)
        elif ats == "Montagna":
            ax.axvspan(idx - 0.45, idx + 0.45,
                       color="#e8f8e8", zorder=0, alpha=0.5)
    ...
    ax.set_title(
        ...
        "Sfondo rosso = ATS Bergamo  |  Sfondo verde = ATS Montagna",
...
def grafico_industriale_ranking(df_all: pd.DataFrame, output_dir: str):
    ...
    e giustificare la scelta di Bergamo e Montagna come estremi.
    """
    df_ind = (
    ...
    colori_bar = [
        "#e74c3c" if ats == "Bergamo" else
        "#27ae60" if ats == "Montagna" else
        "#95a5a6"
    ...
    ax.set_title(
        ...
        "ATS Bergamo (rosso) e ATS Montagna (verde): estremi selezionati",
...
def grafico_mappa_lombardia(risultati: dict, output_dir: str):
    ...
    for ax, label in zip(axes.flatten(), labels_ordine):
        ...
        # Bordo colorato per le ATS selezionate
        if label == "Bergamo":
...
def main():
    ...
    try:
        r_bg    = df_ranking[df_ranking["ats"] == "Bergamo"].iloc[0]
        r_mt    = df_ranking[df_ranking["ats"] == "Montagna"].iloc[0]
        rank_bg = df_ranking[df_ranking["ats"] == "Bergamo"].index[0]
        rank_mt = df_ranking[df_ranking["ats"] == "Montagna"].index[0]

        print(f"\n  ★ DATO CHIAVE PER IL REPORT:")
        print(f"    ATS Bergamo:  {r_bg['percentuale']:.1f}% industriale → "
              f"#{rank_bg} su 8 ATS")
        print(f"    ATS Montagna: {r_mt['percentuale']:.1f}% industriale → "
              f"#{rank_mt} su 8 ATS")
        print(f"    Rapporto: {r_bg['percentuale']/max(r_mt['percentuale'],0.01):.1f}x "
              f"più industriale Bergamo vs Montagna")
    except (IndexError, KeyError):
        print("  ⚠ Bergamo o Montagna non trovate — verificare i label nelle ATS_DEFINIZIONI.")
...
dusaf_ranking_industriale.png  ← FIGURA PRINCIPALE
    Ranking delle 8 ATS per quota industriale. Dimostra che
    Bergamo e Montagna sono i due estremi dello spettro
    lombardo → scelta non arbitraria.
