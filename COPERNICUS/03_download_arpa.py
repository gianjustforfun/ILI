...
NOTA SUI LIMITI API:
  - La query SODA di ARPA ha limiti su URL e numero di risultati
  - Utilizziamo batching: max 30 stazioni per richiesta
"""
...
# Dimensione batch per le richieste API (max 30 stazioni per query)
BATCH_SIZE = 30
...
def scarica_misure_periodo(id_stazioni: list, anno: int, mese: int,
    ...
    ⚠️  IMPORTANTE: usa batching (max ~30 stazioni per richiesta)
    per evitare errori 400 su query troppo grandi.
    """
    ...
    # Dividere in batch e scaricare
    tutti_i_risultati = []
    n_batch = (len(id_stazioni) + BATCH_SIZE - 1) // BATCH_SIZE
    
    for batch_idx in range(n_batch):
        start = batch_idx * BATCH_SIZE
        end = min(start + BATCH_SIZE, len(id_stazioni))
        batch_ids = id_stazioni[start:end]
        
        # Codici stazione come stringa per il filtro
        id_str = ','.join([f"'{sid}'" for sid in batch_ids])
        ...
        try:
            ...
            tutti_i_risultati.append(df)
            
        except requests.exceptions.RequestException as e:
            print(f"    [batch {batch_idx+1}/{n_batch}] ERRORE: {e}")
            continue

    # Concatena tutti i batch
    if tutti_i_risultati:
        return pd.concat(tutti_i_risultati, ignore_index=True)
    else:
        return pd.DataFrame()
...
def main():
    ...
    print(f"Dimensione batch: {BATCH_SIZE} stazioni per richiesta")
    print(f"Numero di batch per mese: ~{(len(id_stazioni) + BATCH_SIZE - 1) // BATCH_SIZE}")
